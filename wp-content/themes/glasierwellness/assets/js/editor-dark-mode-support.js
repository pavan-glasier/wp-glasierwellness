/* global glasierwellnessIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'glasierwellness-supports-dark-theme' ) ) {
	glasierwellnessDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function glasierwellnessDarkModeEditorInit( attempt ) {
	...
	if ( glasierwellnessIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						glasierwellnessDarkModeEditorInit( attempt + 1 );
					},
